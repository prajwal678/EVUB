...
class App {
    ...
    async loginUser(req, res) {
        try {
            ...
            const [results] = await this.connection.execute('SELECT * FROM student WHERE email = ?', [email]);
            if (results.length === 0) {
            ...
            if (!token) {
                ...
                await this.connection.execute('UPDATE student SET token = ? WHERE SRN = ?', [token, user.SRN]);
            }
    ...
    async filterEvents(req, res) {
        try {
            ...
            const [userResults] = await this.connection.execute('SELECT * FROM users WHERE email = ?', [email]);
    ...
    async addEvent(req, res) {
        try {
            ...
            const [clubHeadResults] = await this.connection.execute('SELECT 1 FROM member WHERE memberID = ? AND clubID = ? AND memberID = clubHeadID', [memberID, clubID]);
            ...
            try {
                await this.connection.execute('INSERT INTO event (eventID, eventName, eventDate, eventStartTime, eventEndTime, clubID, venueID) VALUES (?, ?, ?, ?, ?, ?, ?)', [eventID, eventName, eventDate, eventStartTime, eventEndTime, clubID, venueID]);
